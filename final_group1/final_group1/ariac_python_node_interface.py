...
from ariac_msgs.msg import Order as OrderMsg, AGVStatus, CompetitionState, AdvancedLogicalCameraImage, VacuumGripperState, Part
...
class OrderManagement(Node):
    ...
    def __init__(self, node_name):
        ...
        # Initialize the flags for order processing
    ...
    def _orders_initialization_cb(self, msg):
        """
        Callback for receiving orders. Processes the orders based on priority and creates subscriptions to the AGV status topic.

        Args:
            msg (any): Message received from the orders topic
        """
        order = Order(msg)
    ...
    def _bin_camera_callback(self, message,side='Unknown'):
        ...
        if self.bins_done[side] == False:
            ...
            if(len(bin_poses) > 0):
                self.bins_done[side] = True # Set the flag to True if the bins are scanned
                ...
                # Iterate through the bin poses and store the data in the dictionary
                for i in range(len(bin_poses)):
                    ...
                    bin_world_pose = self._multiply_pose(bin_camera_pose, bin_part_pose) # Get the world pose of the bin
                    type = self._Parts_Dictionary['types'][bin_part.type]
                    color = self._Parts_Dictionary['colors'][bin_part.color]
                    
                    # Check if the type and color are already in the dictionary and update the values
    ...
    def _agv_camera_callback(self, message, agv_id):
        ...
        Returns:
            None
        """
        # Check if the AGV ID is unknown
        if agv_id not in self._Agvs_Dictionary:
            self._Agvs_Dictionary[agv_id] = {}
        
        # Check if the AGV poses are already scanned
        ...
        if len(agv_poses) > 0:
            for i in range(len(agv_poses)):
                ...
                part_world_pose = self._multiply_pose(camera_pose, part_pose) # Get the world pose of the part
                
                # Check if the AGV ID is already in the dictionary and update the values
                self._Agvs_Dictionary[agv_id].update({i:{'part':part,'pose':part_world_pose}})

    def _multiply_pose(self, pose1: Pose, pose2: Pose) -> Pose:
        """
        Multiplies two poses and returns the resulting pose.

        Args:
            pose1 (geometry_msgs.msg.Pose): The first pose.
            pose2 (geometry_msgs.msg.Pose): The second pose.

        Returns:
            A `geometry_msgs.msg.Pose` object containing the result of multiplying `pose1` and `pose2`.
        """
    ...
    def _execute_move_it_tasks(self, order):
        ...
        It moves the robot to the home position, picks the tray, places the tray on the AGV, and moves the robot to the table.
        1. Move Robot to Home
        2. Pick Tray and Place on AGV
        3. Move Robot to Table
        4. Pick Parts and Place on Tray
        5. Move Robot to Tray
        6. Move Tray to AGV
        ...
        # Check the priority of the order
    ...
    def _quality_check(self, order_id):
        ...
        Args:
            order_id (str): ID of the order to perform the quality check on
    ...
    def _quality_check_cb(self, future):
        ...
        Returns:
            None
        """
        quadrants = [] # List to store the quadrants
        response = future.result()
        
        # Check if the response is received and update the quadrants
        if response:
            self._valid_id = response.valid_id
            self._all_passed = response.all_passed
            self._incorrect_tray = response.incorrect_tray
    ...
    def _pick_place_parts_on_tray(self, order, agv_id):
        ...
        while True:
            for v in order._parts_status_tray.values():
                if(v["part_status"] == False):
                    ...
                    if part_quadrant in self._faults:
                        ...
                        self.get_logger().info("Part Placed on Tray")
            ...
            # Check if all parts are placed on the tray
    ...
    def _faulty_gripper_challenge(self):
        ...
        # Detach the part from the planning scene
        self._detach_part()
    ...
    def _lock_tray(self, agv):
        ...
        self.get_logger().info("Lock Tray service called")
    ...
    def _move_agv(self, agv, destination):
        ...
        self.get_logger().info("Move AGV service called")
    ...
    def _pick_part_bin_cli_cb(self, future):
        """
        Client callback for the service /commander/pick_part_bin. Prints the result of the service call and sets the flag.
